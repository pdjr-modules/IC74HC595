...
IC74HC595::IC74HC595(unsigned char gpioClock, unsigned char gpioData, unsigned char gpioLatch, unsigned int buffers) {
...
void IC74HC595::write(unsigned int status, unsigned int mask) {
  this->status = (this->status & ~mask) | status;
...
void IC74HC595::callbackMaybe(bool force) {
  ...
  if ((now > deadline) || force) {
    if (this->callback) {
      this->write(this->callback(this->status));
    }
